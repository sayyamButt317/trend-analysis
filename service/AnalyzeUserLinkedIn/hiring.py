    ...
    try:
        jobs_data = await session.page.evaluate(
            ...
            for (const a of document.querySelectorAll('a[href*="/jobs/view/"]')) {
                ...
                if (!title) continue;
                results.push({ job_title: title, linkedin_url: href });
                if (results.length >= limit) break;
            }
            return results;
        }""",
        ...
        for item in jobs_data or []:
            jobs.append({**item, "source": "linkedin_playwright"})
    except Exception:
...
async def fetch_jobs_tavily(company_name: str, *, limit: int) -> list[dict[str, Any]]:
    ...
    if not api_key:
        return []
    slug = company_name_to_slug(company_name)
    query = f'site:linkedin.com/jobs/view "{company_name}"' + (f' "{slug}"' if slug else "") + " hiring"
    jobs: list[dict[str, Any]] = []
    try:
        from tavily import TavilyClient
        ...
        for item in (response or {}).get("results") or []:
            ...
            if job_url in seen:
                continue
            seen.add(job_url)
            title = (item.get("title") or "Open role").strip()
            jobs.append(
                {
                    "job_title": title.split(" | ")[0][:200],
                    "location": None,
