...
async def DiscoverCompetitorsNode(state: CompetitorState) -> CompetitorState:
    ...
    for item in competitors:
        ...
        if not username:
            ...
            username = (socials.get("instagram") or "").strip().lstrip("@").lower()
        if username:
            normalized.append({**item, "username": username})
    ...
    manual_mode = (filters_applied.get("matching_mode") or "") == "manual"
    ...
    posts, profiles_by_username = _collect_prefetched_posts(competitors)
    missing_usernames = [
    ...
    ranked_usernames = {
        (item.get("username") or "").strip().lstrip("@").lower() for item in ranked
    }
