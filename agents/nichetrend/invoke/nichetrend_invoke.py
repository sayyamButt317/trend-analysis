...
from agents.nichetrend.graph.nichetrend_graph import niche_trend_graph_app
...
def _initial_state(agent_config: dict[str, Any]) -> TrendState:
    return {
        "config": agent_config,
...
async def nicheTrendInvoke(
    request: NicheTrendRequest | dict[str, Any],
) -> dict[str, Any]:
    start_time = time.time()

    if isinstance(request, dict):
        request = NicheTrendRequest.model_validate(request)

    agent_config = request.to_agent_config()
    company_payload = request.normalized_company()

    try:
        final_state = await niche_trend_graph_app.ainvoke(_initial_state(agent_config))
        ...
        meta = {
            "status": (
                "success"
                if not has_error and (post_count > 0 or has_web or has_company_analysis)
                else "partial"
                if post_count > 0 or has_web or has_company_analysis
                else "failed"
            ),
